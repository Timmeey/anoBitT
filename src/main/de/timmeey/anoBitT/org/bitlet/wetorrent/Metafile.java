...
import de.timmeey.anoBitT.org.bitlet.wetorrent.bencode.DictionaryComparator;
...
public class Metafile extends Bencode {
	...
	public Metafile(InputStream is) throws IOException,
		...
		byte[] piecesByteString = ((ByteBuffer) info
				.get(toByteBuffer("pieces"))).array();
		...
		ByteBuffer commentByteBuffer = (ByteBuffer) rootDictionary
				.get(toByteBuffer("comment"));
		if (commentByteBuffer != null) {
			comment = new String(commentByteBuffer.array());
		}
		ByteBuffer createdByByteBuffer = (ByteBuffer) rootDictionary
				.get(toByteBuffer("created by"));

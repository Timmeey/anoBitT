...
public class PeersManager {
	 ...
	 */
	public synchronized TorrentPeer offer(byte[] peerId, InetAddress ip,
			int port) {

		// TODO: this could be optimized with a proper indexing
		// Careful here, this only works for plain peerGroupMember sockets
		...
		if (activePeers.size() < connectionCreationTreshold) {
			...
			if (Torrent.verbose) {
				torrent.addEvent(new Event(this,
						"Starting connection to new peer: " + ip, Level.FINE));
			}
			peer.start();
		...
		if (Torrent.verbose) {
			torrent.addEvent(new Event(this,
					"Too many connection, peer refused: " + ip, Level.FINER));
		}
		return null;
	}

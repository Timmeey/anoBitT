		...
		try {
			...
			if (message != null) {
					...
					if (payload != null) {
						...
						while (payloadOffset < payload.length) {
							...
							if (message.getType() == message.PIECE) {
								uploaded += payloadChunk
										- (payloadOffset == 0 ? 13 : 0);
							}
				...
				if (Torrent.verbose) {
					peer.getPeersManager()
							.getTorrent()
							.addEvent(
									new Event(this, "Message sent "
											+ message.getType(), Level.FINEST));
				}
			}
			...
			if (Torrent.verbose) {
				peer.getPeersManager()
						.getTorrent()
						.addEvent(
								new Event(this, "Problem sending message",
										Level.WARNING));
			}
			throw e;
		}
	...
	public synchronized void cancel(int index, int begin, int length) {
		...
		for (Message elem : messagesToBeSent) {
			if (elem.getType() == Message.PIECE) {
				...
				if (block.getIndex() == index && block.getBegin() == begin
